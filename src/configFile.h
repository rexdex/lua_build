...
struct ProjectStructure
{
    ...
    struct FileInfo
    {
        ...
        bool flagNoWarnings = false;
        bool flagWarn3 = false;
        bool flagExcluded = false;

        const ProjectInfo* originalProject = nullptr;
    ...
    struct ProjectInfo
    {
        ...
        bool flagPureDynamicLibrary = false;
        bool flagGlobalInclude = false;
        bool flagModuleRoot = false; // artificial project that is a module root
        bool flagGenerateMain = false;
        bool flagAllowExceptions = false;

        std::string moduleName; // name of the module to create
        ProjectInfo* moduleProject = nullptr;
        std::vector<ProjectInfo*> moduleSourceProjects;
        ...
        std::vector<std::string> localIncludeDirectories;
        ...
        std::vector<std::string> externalIncludePaths; // external include path to add to project, used mostly for headers shared with rendering (constant buffer layouts)
        ...
        void addLocalDefine(std::string_view name, std::string_view value="");
        void addGlobalDefine(std::string_view name, std::string_view value="");
        ...
        void collectModuleDependencies(ProjectInfo* moduleProject, std::vector<ProjectInfo*>& outAllDependencies);
    ...
    private:
        ...
        static int ExportProjectFilter(lua_State* L); // string
        static int ExportProjectModuleName(lua_State* L); // string
        ...
        static int ExportLocalDefine(lua_State* L); // string, string [opt]
        static int ExportLocalIncludeDir(lua_State* L); // string
        static int ExportGlobalDefine(lua_State* L); // string, string [opt]
        ...
        static int ExportExternalIncludeDirectory(lua_State* L); // string
        static int ExportAppMain(lua_State* L); // string, string
        ...
        void scanMediaFilesAtDir(const fs::path& directoryPath);
        bool internalTryAddMediaFileFromPath(const fs::path& absolutePath);
    ...
    bool makeModules(const Configuration& config);
...
struct ScriptProjectStructure
{
    ...
    struct FileInfo
    {
        ...
        fs::path absolutePath; // full path to file on disk "Z:\\InfernoEngine\\engine\\scripts\\mono\\game\\test.cs"
    };
    ...
    struct DirInfo
    {
        ...
        fs::path absolutePath; // full path to file on disk "Z:\\InfernoEngine\\engine\\scripts\\game\\test.cs"

        ~DirInfo();
    };

    std::string name; // "DemoGame"
    fs::path rootPath; // full path to file on disk "Z:\\InfernoEngine\\engine\\scripts\\"
